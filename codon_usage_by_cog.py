...
def main(cds_folder, eggnog_input, output_folder, mode, output_style):

    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # Existing output structure (per COG)
    cog_data = defaultdict(list)

    # New output structure (per genome)
    genome_data = defaultdict(list)
    ...
    for filename in os.listdir(cds_folder):
        ...
        for gene_id, seq in sequences.items():
            ...
            for cog in gene_to_cogs[gene_id]:
                ...
                if output_style == "by_cog":
                    row = [genome_gene_id] + [str(counts[c]) for c in CODONS]
                    cog_data[cog].append(row)

                # New: per-genome output
                else:  # output_style == "by_genome"
                    row = [genome_gene_id, cog] + [str(counts[c]) for c in CODONS]
                    genome_data[genome_name].append(row)
    ...
    if output_style == "by_cog":
        ...
        header = ["genome_geneID", "COG"] + CODONS

        for genome_name, rows in genome_data.items():
            outpath = os.path.join(output_folder, f"{genome_name}.tsv")
            with open(outpath, "w") as out:
                out.write("\t".join(header) + "\n")
                for row in rows:
                    out.write("\t".join(row) + "\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Count codon usage per gene and separate by COG (or by genome)."
    )
    ...
    parser.add_argument("--output_style",
                        choices=["by_cog", "by_genome"],
                        default="by_cog",
                        help="Output as current per-COG files (by_cog) or one file per genome (by_genome)")
